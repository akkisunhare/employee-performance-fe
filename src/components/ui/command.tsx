...
// Explicitly define props for the Command component
interface CommandProps extends React.ComponentProps<typeof CommandPrimitive> {
  // Ensure children match the type expected by CommandPrimitive
  children?: React.ComponentProps<typeof CommandPrimitive>['children'];
}

// Update the Command component signature and implementation
function Command({
  className,
  children, // Destructure children
  ...props
}: CommandProps) { // Use the new interface
  return (
    <CommandPrimitive
      data-slot="command"
      className={cn(
...
// Keep the previous changes to CommandDialogProps for clarity
...
function CommandList({
  ...
  return (
    <CommandPrimitive.List
      ...
      className={cn(
        "max-h-[230px] scroll-py-1 overflow-x-hidden overflow-y-auto",
        className
      )}
      {...props}
    />
